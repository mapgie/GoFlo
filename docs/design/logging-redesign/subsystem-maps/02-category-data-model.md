...
- **Groups / roles**: *(updated v24)* the `Group` entity, `TrackingCategory.groupId`, and the `"inherit"` colour sentinel now exist (Phase 2). No management UI yet — repository-level only until Phase 6. At the original stamp none of this existed; closest prior mechanisms were `modeKey`, `systemKey`, `showInLogPeriod`, `custom_alarm_categories`.
