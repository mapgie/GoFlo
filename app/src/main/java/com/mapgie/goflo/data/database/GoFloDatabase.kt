...
abstract class GoFloDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** Adds the color_profiles table for saved custom palette slots (v21). */
                ...
                database.execSQL(
                    """CREATE TABLE IF NOT EXISTS `color_profiles`
                       (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                        ...
                        `primaryArgb` INTEGER NOT NULL DEFAULT 0,
                        `secondaryArgb` INTEGER NOT NULL DEFAULT 0,
                        `tertiaryArgb` INTEGER NOT NULL DEFAULT 0)"""
                )
            }
        }

        /** Adds per-mode background override columns to color_profiles (v22). 0 = Auto. */
        val MIGRATION_21_22 = object : Migration(21, 22) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE color_profiles ADD COLUMN lightBackgroundArgb INTEGER NOT NULL DEFAULT 0"
                )
                database.execSQL(
                    "ALTER TABLE color_profiles ADD COLUMN darkBackgroundArgb INTEGER NOT NULL DEFAULT 0"
                )
            }
        }
